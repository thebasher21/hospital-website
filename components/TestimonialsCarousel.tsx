...
export default function TestimonialsCarousel({ testimonials }: TestimonialsCarouselProps) {
  ...
  return (
    <div className="max-w-xs sm:max-w-md md:max-w-2xl lg:max-w-4xl mx-auto" ref={containerRef}>
      <Carousel 
        className="w-full" 
      ...
      >
        <CarouselContent className="-ml-2 md:-ml-4">
          {testimonials.map((testimonial, index) => (
            <CarouselItem key={index} className="basis-full md:basis-1/2 pl-2 md:pl-4 pb-6 pt-2">
              <Card className="shadow-md border bg-white dark:bg-gray-800/70 h-full">
                <CardContent className="pt-6 flex flex-col h-full">
                  <p className="italic text-slate-600 dark:text-slate-300 mb-4 flex-grow" dangerouslySetInnerHTML={{ __html: testimonial.quote }} />
      ...
      <div className="mt-6 space-y-4">
        ...
        {/* Dynamic Page Indicators based on number of pages */}
        <div className="flex justify-center gap-2">
          {Array.from({ length: totalPages }).map((_, pageIdx) => {
            // Calculate the active page based on current slide and visible slides
            const activePage = Math.floor(current / visibleSlides);
            return (
              <button
                key={pageIdx}
                ...
                onClick={() => api?.scrollTo(pageIdx * visibleSlides)}
